...
def main(msg, num):
    ...
    # from a file.
    filename = 'archivoencriptado.txt' # The file to write to/read from.
    ...
    try:
        if mode == "1":
            #print ("Escriba el mensaje a encriptar")
            #message = sys.stdin.readline().strip()
            pubKeyFilename = 'generatedKey_pubkey.txt'
            #print('Encriptando y escribiendo a %s...' % (filename))
            encryptedText = EncryptOnly(filename, pubKeyFilename, message)
            #print('Texto Encriptado:')
            #print(encryptedText)
            return encryptedText
        elif mode == '2':
            privKeyFilename = 'generatedKey_privkey.txt'
            #print('Leyendo de %s and decifrando...' % (filename))
            decryptedText = DecryptOnly(message, privKeyFilename)
            #print('Texto cifrado:')
            #print(decryptedText)
...
def EncryptOnly(messageFilename, keyFilename, message, blockSize=None):
    # Using a key from a key file, encrypt the message and save it to a
    # file. Returns the encrypted message string.
    keySize, n, e = readKeyFile(keyFilename)
    if blockSize == None:
...
def DecryptOnly(messageFilename, keyFilename):
    # Using a key from a key file, read an encrypted message from a file
    # and then decrypt it. Returns the decrypted message string.
    keySize, n, d = readKeyFile(keyFilename)
